...
[TERM: Prompt Injection — A class of attack where malicious instructions are embedded in content that an LLM processes, exploiting the model's inability to distinguish between legitimate instructions and injected ones.]
...
[IMAGE: "Ignore all previous instructions" meme — showing a user prompt overriding a system prompt]
...
[TERM: Jailbreaking — A specific form of direct injection where the goal is to bypass a model's built-in safety guardrails — getting it to produce content it's been trained to refuse, or reveal its system prompt.]
...
[IMAGE: Side-by-side showing a clean-looking webpage and its HTML source with hidden injection instructions in comments and white text]
...
[TERM: Second-order injection — An attack where a compromised or manipulated agent passes injected instructions to a peer agent, exploiting trust relationships between agents in a multi-agent system.]
...
[TERM: Principle of Least Privilege — Granting a system only the minimum permissions it needs to perform its intended task, and no more.]
...
When an agent does need access to consequential systems — sending emails, executing code, making API calls that modify state — require human approval before irreversible actions. This is your [TERM: Human-in-the-loop checkpoint — A required human review step before an AI system can execute an irreversible or high-impact action].
...
[IMAGE: "The call was coming from inside the house" — a security scanner icon with a red alert overlay]

[ATTACK MODEL CARD: The LiteLLM Supply Chain Attack — March 24, 2026]
